...
> **Master Standard**: This project strictly follows the 《Godot Game Development Master Protocol (GGD-Protocol-v1)》 located at skills/GODOT_GAME_DEV_PROTOCOL.md.
...
1. **Strict 6-Stage Lifecycle**:
   ...
   - Stage 3: 5-Layer Forward MCP Construction (Autoload $\rightarrow$ Meshes/Mats $\rightarrow$ Nodes/Collisions $\rightarrow$ Typed Scripts $\rightarrow$ Settings)
   - Stage 4: Self-Healing Diagnostics & Multimodal Visual QA (run_project $\rightarrow$ get_editor_errors $\rightarrow$ take_screenshot $\rightarrow$ stop_project)
