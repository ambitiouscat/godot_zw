...
func get_commands() -> Dictionary:
	return {
		...
		"resave_resources": _resave_resources,
	}
...
func _export_mesh_library(params: Dictionary) -> Dictionary:
	var res_scene := require_string(params, "scene_path")
	if res_scene[1] != null:
		return res_scene[1]
	var scene_path: String = res_scene[0]
	if not scene_path.begins_with("res://"):
		scene_path = "res://" + scene_path

	var res_out := require_string(params, "output_path")
	if res_out[1] != null:
		return res_out[1]
	var output_path: String = res_out[0]
	...
	for child: Node in scene_root.get_children():
		...
		if child is MeshInstance3D:
			...
			for desc: Node in child.get_children():
				if desc is MeshInstance3D:
					mesh_instance = desc
					break
		...
		if mesh_instance != null and mesh_instance.mesh != null:
			...
			for col_child: Node in child.get_children():
				if col_child is CollisionShape3D and col_child.shape != null:
					mesh_library.set_item_shapes(item_id, [col_child.shape])
					break
	...
	if item_id == 0:
		return error_invalid_params("No valid MeshInstance3D nodes found in scene '%s'" % scene_path)

	var out_dir: String = output_path.get_base_dir()
	if out_dir != "res://" and not DirAccess.dir_exists_absolute(ProjectSettings.globalize_path(out_dir)):
...
func _find_files_recursive(path: String, extensions: Array[String]) -> Array[String]:
	...
	while file_name != "":
		if dir.current_is_dir():
			if not file_name.begins_with("."):
				found.append_array(_find_files_recursive(path.path_join(file_name), extensions))
		else:
...
func _resave_resources(params: Dictionary) -> Dictionary:
	var base_path: String = params.get("project_path", "res://")
	if not base_path.begins_with("res://"):
		base_path = "res://" + base_path

	var scene_files := _find_files_recursive(base_path, [".tscn"])
	var saved_scenes := 0
	var scene_errors := 0

	for s_path: String in scene_files:
		var loaded_res = ResourceLoader.load(s_path)
		if loaded_res != null:
			if ResourceSaver.save(loaded_res, s_path) == OK:
				saved_scenes += 1
			else:
				scene_errors += 1
		else:
